...
public final class LogRunnerService extends JobIntentService {
     ...
     */
    private static long getMaxDate(@NonNull final ContentResolver cr, final int type) {
     ...
     */
    private static long getLastData(@NonNull final SharedPreferences p, final int type,
                                    final int direction) {
     ...
     */
    @Nullable
    private static Cursor getLastData(@NonNull final ContentResolver cr, final int type,
                                      final int direction) {
            ...
            do {
                ...
                if (cvalues.size() >= CallMeter.HUNDRED) {
                    cr.bulkInsert(DataProvider.Logs.CONTENT_URI,
                            cvalues.toArray(new ContentValues[0]));
                    Log.d(TAG, "new calls: ", cvalues.size());
                    cvalues.clear();
                }
            } while (cursor.moveToNext());
            if (cvalues.size() > 0) {
                cr.bulkInsert(DataProvider.Logs.CONTENT_URI, cvalues.toArray(new ContentValues[0]));
                Log.d(TAG, "new calls: ", cvalues.size());
                Editor e = p.edit();
