...
public final class AskForPlanActivity extends AppCompatActivity
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        tvTimeout = d.findViewById(R.id.autohide);
        cbSetDefault = d.findViewById(R.id.set_default);
        ...
        do {
            ...
            final Button v = d.findViewById(PLAN_BTNS[i]);
        ...
        for (i = 0; i < MAX_PLANS; i++) {
            if (planIds[i] == defaultPlanId) {
                ...
                final Button v = d.findViewById(bid);
                v.requestFocus();
    ...
    static class TimeoutTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onProgressUpdate(final Void... values) {
            String s = context.get().getString(R.string.autohide_in_, count);
            context.get().tvTimeout.setText(s);
        }

        @Override
        protected void onPostExecute(final Void result) {
            if (!this.isCancelled() && !context.get().isFinishing()) {
                Log.i(TAG, "autohide dialog");
                final int pid = context.get().defaultPlanId;
                if (pid >= 0L) {
                    Log.i(TAG, "setPlan(" + pid + ")");
                    RuleMatcher.matchLog(context.get().getContentResolver(), context.get().id, pid);
                }
                context.get().d.cancel();
                context.get().finish();
            }
        }
    }
