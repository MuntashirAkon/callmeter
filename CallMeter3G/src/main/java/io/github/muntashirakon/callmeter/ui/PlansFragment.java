...
import io.github.muntashirakon.callmeter.ui.prefs.PlanEdit;
...
public final class PlansFragment extends ListFragment implements OnClickListener,
    ...
    private static class PlansAdapter extends ResourceCursorAdapter {
        ...
        public void bindView(final View view, final Context context, final Cursor cursor) {
            ...
            if (holder == null) {
                ...
                holder.pbPeriod = view.findViewById(R.id.period_pb);
                holder.pbLimitGreen = view.findViewById(R.id.progressbarLimitGreen);
                holder.pbLimitYellow = view.findViewById(R.id.progressbarLimitYellow);
                holder.pbLimitRed = view.findViewById(R.id.progressbarLimitRed);
                ...
                holder.tvBigtitle = view.findViewById(R.id.bigtitle);
                holder.tvPeriod = view.findViewById(R.id.period);
                holder.tvBilldayLable = view.findViewById(R.id.billday_lable);
                holder.tvTitle = view.findViewById(R.id.normtitle);
                holder.tvData = view.findViewById(R.id.data);
                view.setTag(holder);
            }
            ...
            if (plan.type != DataProvider.TYPE_SPACING && plan.type != DataProvider.TYPE_TITLE) {
                if (plan.hasBa) {
                    ...
                    if (plan.type != DataProvider.TYPE_BILLPERIOD) {
                        if (showTotal) {
                            spb.append(delimiter).append(Common.formatValues(context, plan.now,
                                    plan.type, plan.atCount, plan.atBa, plan.billperiod,
                                    plan.billday, pShowHours));
                        }
                        if (showToday) {
                            spb.insert(
                                    0,
    ...
    public void onPause() {
        super.onPause();
        ((PlansAdapter) requireListAdapter()).save();
    }
    ...
    public void onClick(final View v) {
        if (v.getId() == R.id.import_default) {
    ...
    public boolean onItemLongClick(final AdapterView<?> parent, final View view,
        ...
        builder.setItems(R.array.dialog_edit_plan, (dialog, which) -> {
                    switch (which) {
                        case 0:
    ...
    public Loader<Cursor> onCreateLoader(final int id, final Bundle args) {
        ...
        if (id == UID_DUMMY) {
            ...
            return new CursorLoader(requireActivity(), DataProvider.Plans.CONTENT_URI,
                    DataProvider.Plans.PROJECTION_BASIC, where, null, DataProvider.Plans.ORDER
                    + " ASC");
        } else {
            return new CursorLoader(requireActivity(), DataProvider.Plans.CONTENT_URI_SUM
                    .buildUpon()
    ...
    public void onLoadFinished(@NonNull final Loader<Cursor> loader, final Cursor data) {
        ...
        ignoreQuery = false;
        PlansAdapter adapter = (PlansAdapter) requireListAdapter();
        adapter.save();
    ...
    public void onLoaderReset(@NonNull final Loader<Cursor> loader) {
        ...
        try {
            ((PlansAdapter) requireListAdapter()).swapCursor(null);
        } catch (Exception e) {
