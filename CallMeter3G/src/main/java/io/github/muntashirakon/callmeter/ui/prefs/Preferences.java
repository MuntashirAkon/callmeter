...
import io.github.muntashirakon.callmeter.data.DataProvider.XmlMetaData;
import io.github.muntashirakon.callmeter.data.Device;
import io.github.muntashirakon.callmeter.data.ExportProvider;
 ...
 * @author flx
 */
public final class Preferences extends PreferenceActivity implements OnPreferenceClickListener {
    /**
     * Tag for output.
     */
    ...
    public static String getDateFormat(final Context context) {
        final SharedPreferences p = PreferenceManager.getDefaultSharedPreferences(context);
        final String pcs = Objects.requireNonNull(p.getString(PREFS_DATE_FORMAT, ""))
                .replaceAll("\\$", "%");
        if (pcs.length() == 0) {
            return null;
        } else {
    ...
    private void resetDataDialog() {
        ...
        builder.setNeutralButton(R.string.reset_data_data_, (dialog, which) ->
     ...
     */
    @Nullable
    private InputStream getStream(final ContentResolver cr, @NonNull final Uri uri)
            throws IOException {
            ...
            protected void onPostExecute(final String result) {
                ...
                builder.setPositiveButton(android.R.string.ok, (dialog, which) -> {
                    ...
                    new AsyncTask<Void, Void, Boolean>() {
                        ...
                        @Override
                        protected void onPostExecute(final Boolean result1) {
                            try {
                                if (!result1) {
